...
class LeastConnections:
    def __init__(self, servers):
        self.servers = servers
        self.servers_connections = {server: 0 for server in servers}

    def select_server(self):
        for server in self.servers:
            if self.servers_connections[server] == min(self.servers_connections.values()):
                sel_server = server
                break

        self.servers_connections[sel_server] += 1
        return sel_server
    
    def update(self, *arg):
        self.servers_connections[arg[0]] -= 1
...
class LeastResponseTime:
    def __init__(self, servers):
        self.servers = servers
        self.servers_response_time = {server: 0 for server in servers}
        self.servers_times = {server: [0,0] for server in servers}
        self.server_conns = {server: 0 for server in servers}
        self.smalltime = 1*10**(-10)

    def select_server(self):
        # get server with smallest response time
        for server in self.servers:
            if self.servers_response_time[server] == min(self.servers_response_time.values()):
                self.servers_times[server][0] = time.time()
                self.servers_response_time[server] += self.smalltime
                return server

    def update(self, *arg):
        # update response time
        server = arg[0]
        conns = self.server_conns[server] + 1
        self.server_conns[server] = conns

        self.servers_times[server][1] = time.time()
        self.servers_response_time[server] = (
            ((self.servers_response_time[server] * conns) - self.smalltime * conns + (self.servers_times[server][1] - self.servers_times[server][0]))/conns
        )
